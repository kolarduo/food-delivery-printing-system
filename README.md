...
轻量级 Windows 桌面程序，用于读取、展示并打印 Rocket Now Manager 订单。当前版本为 `0.2.1`。
...
- 会缓存已读订单；下次刷新读到本地已有订单后会停止继续翻页，并合并显示本地记录。
...
- 打印格式按 58mm 宽标签纸设计，高度随内容自动变化；主商品大号粗体，选项/小料小号显示在对应商品下方。
- 以单个 Windows EXE 发布，无需解压其他文件。

## 使用方法

1. 运行 `FoodDeliveryPrintingSystem-0.2.1.exe`。
2. 点击右上角“刷新订单”。
...
```text
dist\FoodDeliveryPrintingSystem-0.2.1.exe
```
